...
(function (global, factory) {
    ...
    onDOMContentLoaded(() => {
        ...
        button.forEach(btn => {
            btn.addEventListener('click', event => {
                ...
                if (target?.getAttribute('href') === '#' || targetLink
                    ?.getAttribute('href') === '#') {
                    event.preventDefault();
                }
                if (targetItem) {
                    ...
                    const animationSpeedAttr = lteToggleElement.dataset
                        .animationSpeed;
                    // Build config from data attributes, fallback to Default
                    const config = {
                        accordion: accordionAttr === undefined ? Default
                            .accordion : accordionAttr === 'true',
                        animationSpeed: animationSpeedAttr === undefined ?
                            Default.animationSpeed : Number(animationSpeedAttr)
                    };
    ...
    class PushMenu {
        ...
        addSidebarBreakPoint() {
            ...
            const sidebarExpand = Array.from(sidebarExpandList).find(className => className.startsWith(
                CLASS_NAME_SIDEBAR_EXPAND)) ?? '';
            const sidebar = document.getElementsByClassName(sidebarExpand)[0];
            const sidebarContent = globalThis.getComputedStyle(sidebar, '::before').getPropertyValue(
                'content');
            this._config = {
                ...this._config,
            ...
            if (window.innerWidth <= this._config.sidebarBreakpoint) {
                ...
                if (document.body.classList.contains(CLASS_NAME_SIDEBAR_MINI) && document.body.classList
                    .contains(CLASS_NAME_SIDEBAR_COLLAPSE)) {
                    this.collapse();
                }
            }
        }
    ...
    class AccessibilityManager {
        ...
        initKeyboardNavigation() {
            ...
            document.addEventListener('keydown', (event) => {
                ...
                if ((event.key === 'Enter' || event.key === ' ') && target.hasAttribute(
                    'role') && target.getAttribute('role') === 'button' && !target.matches(
        ...
        handleMenuNavigation(event) {
            ...
            const menuItems = Array.from(currentElement.closest('.nav, .navbar-nav, .dropdown-menu')
      ...
      `;
                    ...
                    mutation.addedNodes.forEach((node) => {
                        if (node.nodeType === Node.ELEMENT_NODE) {
                            ...
                            if (element.matches('.alert-success, .success')) {
                                this.announce(element.textContent || 'Success',
                                    'polite');
                            }
                        }
                    });
        ...
        initFormAccessibility() {
            document.querySelectorAll('input, select, textarea').forEach((input) => {
                ...
                if (!htmlInput.labels?.length && !htmlInput.hasAttribute('aria-label') && !
        ...
        data: {
            ...
            datasets: [
                ...
                },
                {
                    label: 'Garantias',
                    data: [
                        garantiasPorMes[1], garantiasPorMes[2], garantiasPorMes[3], garantiasPorMes[4],
                        garantiasPorMes[5], garantiasPorMes[6], garantiasPorMes[7], garantiasPorMes[8],
                        garantiasPorMes[9], garantiasPorMes[10], garantiasPorMes[11], garantiasPorMes[12]
                    ], // Usando o valor de "garantia" para todos os meses
